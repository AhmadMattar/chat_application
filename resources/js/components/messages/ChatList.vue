                    ...
                    <div class="row gx-5">
                        <div class="col-auto">
                            <div class="avatar" :class="{ 'avatar-online' : conversation.participants[0].isOnline }">
                                <img class="avatar-img" v-bind:src="conversation.participants[0].avatar_url">
                            </div>
                        </div>
                        ...
                        <div class="col">
                            <div class="d-flex align-items-center mb-3">
                                ...
                                <span
                                    class="text-muted extra-small ms-2">{{ $root.moment(conversation.last_message.created_at).fromNow() }}</span>
                            </div>

                            <div class="d-flex align-items-center">
                                <div class="line-clamp me-auto">
                                    {{conversation.last_message.type == 'text' ? conversation.last_message.body : conversation.last_message.body.file_name}}
                                </div>
...
export default {
    ...
    mounted() {
        fetch('api/conversations')
            ...
            .then(json => {
                for(let i in json){
                    json[i].participants[0].isOnline = false;
                }
                this.$root.conversations = json;
            });
    }
}
</script>
