...
const getConversations = function (){
    $.get('api/conversations', function (response){
        for(i in response)
        conversations(response[i]);
    });
}

conversations = function (conversation){
    $('#chat-list').append(`
        <a href="#${conversation.id}" data-conversation_id="${conversation.id}" class="card border-0 text-reset">
            <div class="card-body">
                <div class="row gx-5">
                    <div class="col-auto">
...
$('#chat-list').on('click', '[data-conversation_id]', function (e){
    e.preventDefault();
    let id = $(this).attr('data-conversation_id');
    $('input[name=conversation_id]').val(id);
    $('#chat-body').empty();
    $.get(`/api/conversations/${id}/messages`, function (response){
        $('#chat-name').text(response.conversations.participants[0].name);
        $('#chat-avatar').attr('src', response.conversations.participants[0].avatar_url);
        for (const i in response.messages.data) {
            let msg = response.messages.data[i];
            let c = user_id == msg.user_id ? 'message-out' : '';
            addMessage(msg, c);
        }
    });
});
